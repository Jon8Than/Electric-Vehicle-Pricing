...
ec_train_res3 <- predict(class_tree_final_fit, new_data = ec_test %>% dplyr::select(-price_euro))
...
augment(class_tree_final_fit, new_data = ec_test) %>%
  rsq(truth = price_euro, estimate = .pred)
```
### Boosted Model
```{r}
...
ec_train_res4 <- predict(best_boost_final_model_fit, new_data = ec_test %>% dplyr::select(-price_euro))
ec_train_res4 <- bind_cols(ec_train_res4, ec_test %>% dplyr::select(price_euro))
ec_train_res4 %>%
  head()
...
augment(best_boost_final_model_fit, new_data = ec_test) %>%
  rsq(truth = price_euro, estimate = .pred)
```